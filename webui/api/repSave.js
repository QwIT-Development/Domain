...
const repSave = async (req) => {
  ...
  if (!id || isNaN(score)) {
    return new Response(
      JSON.stringify({
        error: "Invalid request, missing id or score is not a number",
      }),
      { status: 400, headers: { "Content-Type": "application/json" } },
    );
  }
  ...
  if (numericScore < -1000) {
    return new Response(JSON.stringify({ error: "Score too low" }), {
      status: 400,
      headers: { "Content-Type": "application/json" },
    });
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Failed to save reputation" }),
