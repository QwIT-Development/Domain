...
const removeUserDataHandler = require("./api/removeUserData");
...
const getUsersForManagement = require("./func/getUsersForManagement");
...
async function handleApiRoutes(req, pathname) {
  ...
  } else if (req.method === "DELETE" && pathname === "/api/user/remove-data") {
    return await removeUserDataHandler(req);
  ...
  async fetch(req, server) {
    ...
    try {
      ...
      if (pathname.startsWith("/css/")) {
        ...
        const userManagementData = await getUsersForManagement();
