...
const bannedSitesCacheDir = path.join(
  global.dirname,
  "data",
  "running",
  "bannedSitesCache",
...
if (!fs.existsSync(bannedSitesCacheDir)) {
  try {
    ...
    log(
      `Created cache directory: ${bannedSitesCacheDir}`,
      "info",
      "bannedSiteGen.js",
    );
  } catch (err) {
    console.error(
      `Failed to create cache directory ${bannedSitesCacheDir}: ${err}`,
    );
  }
}
...
async function getBannedSites() {
  ...
  try {
    ...
    if (fs.existsSync(staticFilePath)) {
      ...
      log(
        `Blocking ${staticFileSites.length} domains with local filter list.`,
        "info",
        "bannedSiteGen.js",
      );
    }
  ...
  for (const listUrl of remoteLists) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `Error reading cache file ${cachePath}: ${cacheReadError}`,
        );
      }

      if (fetchedContent !== cachedContent) {
        log(
          `Content for ${listUrl} changed or cache miss. Updating cache...`,
          "info",
          "bannedSiteGen.js",
        );
        try {
          ...
          log(
            `Successfully updated cache file: ${cachePath}`,
            "info",
            "bannedSiteGen.js",
          );
        } catch (writeError) {
          console.error(
            `Failed to write cache file ${cachePath}: ${writeError}`,
          );
        }
        currentListSites = fetchedContent
          .split("\n")
          .map((site) => site.trim())
          .filter((site) => site !== "");
      } else {
        log(`Using cached list for ${listUrl}.`, "info", "bannedSiteGen.js");
        currentListSites = cachedContent
          .split("\n")
          .map((site) => site.trim())
      ...
      try {
        if (fs.existsSync(cachePath)) {
          ...
          currentListSites = cachedContent
            ...
            .filter((site) => site !== "");
          log(
            `Successfully loaded ${currentListSites.length} sites from cache for ${listUrl}.`,
            "info",
            "bannedSiteGen.js",
          );
        } else {
        ...
        console.error(
          `Failed to read cache file ${cachePath} after fetch failure: ${cacheError}`,
        );
        currentListSites = [];
      }
    }
  ...
  log(
    `Total domains we are blocking: ${combinedSites.length}`,
    "info",
    "bannedSiteGen.js",
  );
  return true;
