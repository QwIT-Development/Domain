...
async function leaveUnknownServers(client) {
  client.guilds.cache.forEach((guild) => {
    ...
    const guildShouldBeLeft = !channels.some((channelId) =>
      configChannelIds.includes(channelId),
    );

    if (guildShouldBeLeft) {
      console.log(
        `Leaving guild: ${guild.name} (${guild.id}) because none of its channels are in the config.`,
      );
      guild
        .leave()
        .then(() =>
        ...
        .catch((error) =>
          console.error(
            `Failed to leave guild: ${guild.name} (${guild.id})`,
            error,
          ),
        );
    }
