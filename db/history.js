...
async function saveHistory(channelId, history) {
  ...
  try {
    ...
    log(
      `Error saving history for channel ${channelId}: ${error}`,
      "error",
      "prisma/history.js",
    );
  }
...
async function loadHistory(channelId) {
  try {
    ...
    if (Array.isArray(history) && history.length > 0) {
      console.log(
        `Reloaded history for channel ${channelId} with ${history.length} messages.`,
      );
      return history;
    }
    return [];
    ...
    log(
      `Error loading history for channel ${channelId}: ${error}`,
      "error",
      "prisma/history.js",
    );
    return [];
...
async function deleteHistory(channelId) {
  try {
    ...
    if (error.code !== "P2025") {
      log(
        `Error deleting history for channel ${channelId}: ${error}`,
        "error",
        "prisma/history.js",
      );
    }
  ...
  try {
    ...
    for (const result of results) {
      ...
      if (cleanedHistory.length > 0) {
        ...
        console.log(
          `Reloaded history for channel ${result.channelId} with ${cleanedHistory.length} messages.`,
        );
      }
    }
    return histories;
